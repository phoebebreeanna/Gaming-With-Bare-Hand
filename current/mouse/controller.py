...
def draw_distance_ui(img, dist, has_hand):
    ...
    if too_close:   msg, color = "Move FARTHER from camera",    (0,  80, 255)
    elif too_far:   msg, color = "Move CLOSER to camera",       (0, 200, 255)
...
def draw_calibration_ui(img, elapsed, total, trail, has_hand, calib_pts_x, calib_pts_y):
    ...
    cx = (w - tw) // 2
    cy = h // 2 + th // 2
    cv2.putText(img, text, (cx + 3, cy + 3), cv2.FONT_HERSHEY_SIMPLEX, fs, (0, 0, 0), th_ + 2)
    cv2.putText(img, text, (cx, cy), cv2.FONT_HERSHEY_SIMPLEX, fs,
                (0, int(255 * frac), int(255 * (1 - frac))), th_)
    ...
    if not has_hand:
        ...
        cv2.putText(img, "No hand detected!", (10, h - 12),
...
def draw_running_ui(img, gesture, conf, dist_ok, scroll_entry_t, scroll_active,
...
while cap.isOpened():
    ...
    if app_state == 'distance_check':
        ...
        if lms:
            draw_hand(display, lms)
        draw_calib_done_ui(display, max(0.0, 2.0 - elapsed_done))
        if elapsed_done >= 2.0:
            app_state = 'running'
